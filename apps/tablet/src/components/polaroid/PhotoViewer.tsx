...
export default function PhotoViewer({
	...
	const handleEditWithAI = () => {
		if (!isTablet) {
			console.log("[PhotoViewer] Starting AI edit with pre-compressed image");
			sendWs({
				type: "start_ai_edit",
				image: currentImage,
			});
			...
			<div className="relative flex h-full w-full flex-col items-center justify-center p-4 md:p-6">
				...
				>
					...
					>
						...
						<div
							className="absolute inset-0 bg-gradient-to-r from-blue-500 via-indigo-500 to-blue-500 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
							style={{
						...
						<div className="absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
							<div
								className="absolute top-2 left-8 h-1 w-1 rounded-full bg-white/80 animate-ping"
								style={{ animationDelay: "0s" }}
							/>
							<div
								className="absolute top-4 right-12 h-1 w-1 rounded-full bg-white/80 animate-ping"
								style={{ animationDelay: "0.3s" }}
							/>
							<div
								className="absolute bottom-3 left-16 h-1 w-1 rounded-full bg-white/80 animate-ping"
							...
							{isEditingWithAI ? (
								<>
									...
									<span className="tracking-wide">
										Editing with{" "}
										<span className="font-orbitron font-bold bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent">
											Orbit AI
										</span>
										...
									</span>
